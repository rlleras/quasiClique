...
2010/11/19
 Enhanced version that considers pscore_RAxML (should have .fna.summary.pscore_added.RAxML_added

2010/12/06
 Enhanced version that also adds pscore_RAxML_fixTreeEstBranch
"""

def main():
	os.chdir(MOTIF_DIR)
	fieldnames = ['FAMILY', 'FAMSIZE', 'MOTIF', \
			'RANK', 'PSCORE', 'PSCORE_RAxML', 'PSCORE_RAxML_fixTreeEstBranch', \
			'TOTAL_RECALL', 'TPS', 'FPS']
	...
	for d in os.listdir(os.path.curdir):
		...
		for obj in csv.DictReader(\
				open(os.path.join(d, d + '.fna.summary.pscore_added.RAxML_added')),\
				delimiter=','):
		...
		for file in glob.iglob(d + "/*.cmsearched"):
			...
			out_csv.writerow([fam, \
					...
					motif_info[motif_name]['pscore_RAxML'],\
					motif_info[motif_name]['pscore_RAxML_fixTreeEstBranch'],\
					tp_count,\
					",".join(tp),\
					",".join(fp)])
