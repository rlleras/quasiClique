...
R_SCRIPT = os.path.join(os.environ['LCODE'], 'draw_boxplot_score_vs_CMhits_RAxML.R')
...
def main(all_filename='ALL.score_vs_CMhits.evaled', delimiter='\t'):
	...
	"""
	fieldnames = ['MOTIF', 'RANK', 'PSCORE', 'PSCORE_RAxML', \
			'PSCORE_RAxML_fixTreeEstBranch', 'TOTAL_RECALL', 'TPS', 'FPS']
...
def draw(pscore_or_rank='pscore'):
	...
	sort it by rank (field 2) or pscore (field 3) or pscore_RAxML (field 4)
	...
	for file in glob.iglob(pattern):
		...
		elif pscore_or_rank == 'pscore-RAxML':
			output = file + '.pscore_RAxML_sorted'
			os.system("sort -g -k 4 {input} > {output}".format(\
					input=file, output=output))
		elif pscore_or_rank == 'rank':
			output = file + '.rank_sorted'
			os.system("sort -g -k 2 {input} > {output}".format(\
					input=file, output=output))
		else: raise Exception, "pscore_or_rank must be either 'pscore' or 'pscore-RAxML' or 'rank'!"
...
def calc_MCC(filename, fam, output_prefix, db_summary, score_cutoff=0.):
	...
	f_out.write("MOTIF\tRANK\tPSCORE\tPSCORE_RAxML\tPSCORE_RAxML_fixTreeEstBranch\tTP\tFP\tFN\tTN\tMCC\tMyScore\n")
	...
	with open(filename) as f:
		...
		for line in f:
			...
			if len(raw) == 8:
				motif_name, rank, pscore, pscore_RAxML, pscore_RAxML_fixTreeEstBranch, tp_count, tps, fps = raw
			elif len(raw) == 7:
				motif_name, rank, pscore, pscore_RAxML, pscore_RAxML_fixTreeEstBranch, tp_count, tps = raw
				fps = ''
			elif len(raw) == 6:
				motif_name, rank, pscore, pscore_RAxML, pscore_RAxML_fixTreeEstBranch, tp_count = raw
				tps = fps = ''
			else:
			...
			f_out.write(str(motif_name) + '\t' + str(rank)  + '\t' + str(pscore) + '\t' + str(pscore_RAxML) + '\t' + str(pscore_RAxML_fixTreeEstBranch) + '\t' + str(TP) + '\t' + str(FP) + '\t' + str(FN) + '\t' + str(TN) + '\t' + str(MCC) + '\t' + str(myscore) + '\n')
...
#	main()
	...
	# ------------------------------------------------------------------
#	draw('pscore')  # usage: draw('rank|pscore|pscore-RAxML')
#	draw('rank')
#	draw('pscore-RAxML')
#	sys.exit(-1)
	# ------------------------------------------------------------------
	# ------------------------------------------------------------------
	p_suffix = SUFFIX + '.pscore_RAxML_sorted'
